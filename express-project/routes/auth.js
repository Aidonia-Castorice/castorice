...
router.get('/check-user-id', async (req, res) => {
  try {
    const { user_id } = req.query; // 前端传过来的UID
    if (!user_id) {
      return res.status(HTTP_STATUS.BAD_REQUEST).json({ code: RESPONSE_CODES.VALIDATION_ERROR, message: '请输入UID' });
    }
    // 查数据库是否已有该ID
    ...
    res.json({
      ...
      message: existingUser.length > 0 ? 'UID已存在' : 'UID可用'
    });
  } catch (error) {
...
router.post('/register', async (req, res) => {
  try {
    ...
    if (user_id.length < 3 || user_id.length > 15) {
      return res.status(HTTP_STATUS.BAD_REQUEST).json({ code: RESPONSE_CODES.VALIDATION_ERROR, message: 'UID长度必须在3-15位之间' });
    }

    if (!/^[a-zA-Z0-9_]+$/.test(user_id)) {
      return res.status(HTTP_STATUS.BAD_REQUEST).json({ code: RESPONSE_CODES.VALIDATION_ERROR, message: 'UID只能包含字母、数字和下划线' });
    }
    ...
    console.log(`用户注册成功 - 用户ID: ${userId}, UID: ${userRows[0].user_id}`);
...
router.post('/login', async (req, res) => {
  try {
    ...
    console.log(`用户登录成功 - 用户ID: ${user.id}, UID: ${user.user_id}`);
